                    ...
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            if (isset($_SESSION['user_id'])) {
                                ...
                                $book_guide_id = $row['id'];

                                $stmt = $conn->prepare("SELECT * FROM tbl_book_guide_payment WHERE user_id = ? AND book_guide_id = ? AND payment_status = 'completed'");
                                $stmt->bind_param("ii", $user_id, $book_guide_id);
                                $stmt->execute();
                                $paymentResult = $stmt->get_result();

                                if ($paymentResult->num_rows > 0) {
                                    // Here you can link the attachment if it's available
                                    // For attachment handling (if the guide has an attachment)
                                    $attachment = $row['attachment_url'];
                                    if (!empty($attachment) && file_exists($attachment)) {
                                        echo '<a href="downloadBookGuide.php?id=' . $book_guide_id . '" class="btn btn-success mt-2">Download</a>';
                                    } else {
                                        echo '<p class="text-danger mt-2">No attachment available</p>';
                                    }
                                } else {
                                    echo '<a href="payment-form-ui.php?book_guide_id=' . $book_guide_id . '" class="btn btn-success">Purchase</a>';
                                }
                            } else {
